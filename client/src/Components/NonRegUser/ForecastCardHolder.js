...
const ForecastCardHolder = () => {
  ...
  const filterForecastForNextFiveDays = (forecast) => {
    ...
    const nextFiveDays = new Date(currentDate.getTime() + 5 * 24 * 60 * 60 * 1000); // Calculate five days from today
    ...
    for (const item of forecast) {
      ...
      if (forecastDate >= currentDate && forecastDate <= nextFiveDays) {
        const existingDateIndex = filteredForecast.findIndex(
          (filteredItem) => new Date(filteredItem.date).toDateString() === forecastDate.toDateString()
        );
